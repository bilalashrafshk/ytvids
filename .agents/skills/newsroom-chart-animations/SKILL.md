...
description: Use for designing, building, reviewing, or revising evidence-led newsroom charts, graphs, timelines, counters, rankings, ledgers, and other data animations in Remotion. Requires a Remotion project preflight and covers reference research, editorial design, data integrity, motion grammar, sourcing, and multi-format QA.
---
...
Model repeatable roles as named arrays, not fixed subject-specific fields. Each event should normally expose `name`, `start`, `duration` or `end`, and only the parameters its renderer needs.
