...
| **CP-4: Shot Duration Bounds & Static Pacing Rule** | Phase 4 Beats | Every individual beat duration $D_i$ must target an average of **$2.5\text{s} - 4.0\text{s}$**. **Static Image Hard Ceiling:** Strict maximum **$6.0\text{s}$** (rare absolute maximum **$7.0\text{s}$** reserved exclusively for high-detail forensic charts/filings). Fast cuts/punch-ins: $1.5\text{s} - 3.0\text{s}$. **NO Hard Cap on Remotion Cinematics:** Remotion motion graphics, 3D aerial flyovers, infinite zoom tunnels, and multi-step waterfalls have **NO artificial 7.0s hard cap**; they can run **$4.0\text{s} - 12.0\text{s}+$** as long as the animation is continuously evolving and locked to the spoken narration beat. **Anti-Static Document Rule:** Strictly prohibit static holds on dense text/documents; all contracts, deeds, and SEC filings must deploy animated Remotion highlights (`ARCHETYPE_NEWSPRINT_EDITORIAL`), typewriter reveals, or rubber stamps. | Zero static beats $> 7.0\text{s}$ (instant failure due to visual stagnation); zero beats $< 1.5\text{s}$; zero un-animated text-heavy documents. Remotion clips dynamically sized to narration ($4\text{s}-12\text{s}+$). |
...
3. **Character Setup**
4. **Voice Direction & Master VO Audio Generation** — turn the locked script into chunked VoxCPM2 JSON (`VOICE_DIRECTION_VOXCPM2.json` / `05_VOICE_DIRECTION.json`) using the **Two-Part Control Instruction Protocol** (prepending the script's format-locked Persona Anchor to every chunk, and modulating only pacing/restraint without shouting), strictly enforcing phonetic decimal normalization (`1 point 2`, never raw `1.2`), generate the master audio track, and output the timestamped sentence/phrase alignment JSON.
5. **Beat-by-Beat Generation** (Part A body — requires CP-3, CP-4, CP-5, CP-6, CP-7, CP-8, CP-9 audit; uses VO timestamp alignment for 0.0s drift)
6. **Consolidated Batches** (Part A's grouped-by-type execution deliverable — requires CP-10 audit)
7. **Shorts Spinoffs (`shorts.md`)** (optional, additional to the core episode) — 3-4 vertical (9:16) short-form pieces derived from the script and reusing long-form 4K assets. Must be generated as a dedicated standalone markdown file saved directly to `shorts.md` in the episode project directory, containing full vertical scripts, NotebookLM Video Overview prompts, and CapCut vertical assembly instructions.
8. **Part B — Editing & Assembly** — a fully separate deliverable, only generated once every file from Phases 1-6's batches actually exists. Never bundled into the same output as Part A, never generated early.
...
3. **Pacing & Shot Duration Bounds ($D$):** Every beat must target an average of **$2.5\text{s} - 4.0\text{s}$**.
   - **Static Image Ceiling:** Strict maximum **$6.0\text{s}$**; rare absolute ceiling **$7.0\text{s}$** reserved only for high-density forensic charts/documents.
   - **Motion Assets (AI Video):** $4.0\text{s} - 7.0\text{s}$.
   - **Remotion Motion Graphics & Cinematics:** **NO hard 7.0s cap.** Sized dynamically to the spoken narration beat ($4.0\text{s} - 12.0\text{s}+$); continuous 3D aerial flyovers, infinite recursive zoom tunnels, and multi-step waterfalls stay captivating and cool at extended durations because the visual field is continuously evolving without stagnation.
...
**The Inviolable Coverage Rule:** The sum of all individual beat durations ($\sum D_i$) must equal the total narration runtime ($T$) within $\pm 2\%$. An engine generation that produces fewer than $B_{min}$ beats (for example, generating only 35–45 beats for an 840-second script, forcing shots to hold for 20+ seconds) is a **CORRUPT DELIVERABLE AND AN AUTOMATIC SYSTEM FAILURE**. Every sentence of the narration must have an assigned visual asset synchronized to its natural spoken duration.
...
**MANDATORY DELIVERABLE FILE:** Shorts Spinoffs MUST be generated and saved as a dedicated, standalone markdown file: **`shorts.md`** in the episode project directory (e.g., `videos/01-peloton-collapse/shorts.md`), NEVER bundled inside the long-form `03_PRODUCTION_DOCUMENT.md`. This isolates short-form execution from the master long-form assembly and keeps production clean and modular.
...
Format Declaration → File-naming convention → Video technical spec → **Part A, phase by phase, each a stopped deliverable:** Titles & Core Narrative Hook → Thumbnail Concepts → Character Setup → Voice Direction & Master VO Audio → Beat-by-Beat Generation → Consolidated Batches → Shorts Spinoffs (saved directly to standalone `shorts.md`) → **Part B, as a fully separate deliverable once Part A's assets exist:** Beat-by-Beat Assembly Guide & CapCut Timeline Integration → **Final Deliverable:** Verified YouTube Video Description & Accurate Chapter Timestamps (derived directly from final captions & video cuts).

# **FinanceCraft — Voice Direction Prompt (VoxCPM2)**

*The auditory spine of the pipeline. Feed this a finished narration script from the Script Generator. It turns the script into a chunked, VoxCPM2-ready voiceover generation script (`VOICE_DIRECTION_VOXCPM2.json` / `05_VOICE_DIRECTION.json`). Enforces the **Two-Part Control Instruction Protocol**: locking one unchanging Persona Anchor across all chunks (mapped from the episode's core archetype) and modulating only delivery registers (cadence, pacing, quiet weight) without shouting. Generating the master VO audio and its timestamped alignment JSON here provides the ground-truth timing for Phase 4's Beat-by-Beat generation, ensuring visual cuts snap to spoken cadence with zero drift.*
