...
> | **TASK-10** | **Idea Generation & Topic Discovery** | `# FinanceCraft — Idea Generation Prompt` | `Lines 1866–1904` | Outlier topic discovery, viral economic anomalies. |
...
3. **Pacing & Shot Duration Bounds ($D$):** Every beat must target an average of **$2.5\text{s} - 4.0\text{s}$**.
   - **Static Image Pacing Standard:** Default scene stills target **$2.5\text{s} - 5.0\text{s}$**; generic stills $> 6.0\text{s}$ are actively discouraged to maintain narrative momentum. **The Nuanced Evidentiary & Emphasis Exception ($6.0\text{s} - 10.0\text{s}$):** Permitted when a beat carries dense forensic reading material (SEC filings, deeds, balance sheets, contract exhibits) or high-stakes dramatic revelations where the audience needs cognitive time to absorb the claim while the narrator speaks. **Motion Requirement (Never Frozen):** Any static image held for $6.0\text{s} - 10.0\text{s}$ MUST deploy continuous slow Ken Burns push-in ($1.05\times - 1.15\times$) or layered 2.5D parallax so the screen remains alive.
